...
class CarVisualizer:
    def __init__(self, csv_path, width=1920, height=1080):
        ...
        # Vertical offset tuning parameters
        self.base_offset = 0.7  # Base vertical offset in meters
        self.reference_distance = 100.0  # Distance where offset = base_offset
        self.offset_scale_power = 1.0  # Power for non-linear scaling (1.0 = linear)
    ...
    def project_to_screen(self, car_pos, camera_pos, camera_look, fov):
        ...
        # Base offset for "medium" distance (e.g., 50 meters)
        base_offset = self.base_offset
        reference_distance = self.reference_distance
        
        # Scale offset based on distance with power curve
        # Power > 1 means more offset at far distances
        # Power < 1 means less offset variation
        dynamic_offset = base_offset * pow(distance / reference_distance, self.offset_scale_power)
        
        # Apply vertical offset
        adjusted_car_pos = car_pos + np.array([0, dynamic_offset, 0])
        
        # Calculate vector from camera to car
        to_car = adjusted_car_pos - camera_pos
